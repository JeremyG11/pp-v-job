...
export const TweetsController = async (req: Request, res: Response) => {
  try {
    ...
    // Sanitize the AI response to remove markdown code fences.
    ...
    const sanitizedResponse = sanitizeJSON(aiResponse);

    type AIResponse = {
      responses: { type: string; text: string }[];
    };

    const parsed: AIResponse = JSON.parse(sanitizedResponse);

    const responsesArray = parsed.responses.map((response) => ({
      responseType: response.type,
      responseText: response.text,
    }));

    console.log("Parsed responses:", responsesArray);
    ...
    await db.$transaction(
      responsesArray.map(({ responseText, responseType }) =>
        db.generatedTweetResponse.create({
          data: {
            tweetId, // provided tweet id
            engagementType, // provided engagement type (e.g., "HUMOR")
            responseType, // e.g., "Playful", "Witty", etc.
            response: responseText, // the actual tweet text generated by the AI
          },
        })
      )
    );
